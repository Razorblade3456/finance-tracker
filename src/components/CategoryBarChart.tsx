...
interface CategoryBarChartProps {
  ...
  summaryValue?: number;
  activeSummaryHelper?: (bar: ChartDatum & { percentage: number }) => string;
  emptyMessage?: string;
  ariaLabel?: string;
}
...
export function CategoryBarChart({
  ...
  summaryHelper = 'Across all categories',
  summaryValue,
  activeSummaryHelper,
  emptyMessage = 'No category spending to visualize yet.',
  ...
  const summaryLabelText = activeBar ? activeBar.name : summaryLabel;
  const summaryValueText = activeBar ? activeBar.value : summaryValue ?? total;
  const summaryHelperText = activeBar
    ? activeSummaryHelper
      ? activeSummaryHelper(activeBar)
  ...
  return (
    <div className="bar-chart" role="img" aria-label={ariaLabel}>
      ...
      <div className="bar-chart-summary">
        <span className="bar-chart-summary-label">{summaryLabelText}</span>
        <span className="bar-chart-summary-value">{formatCurrency(summaryValueText)}</span>
        <span className="bar-chart-summary-helper">{summaryHelperText}</span>
      </div>
